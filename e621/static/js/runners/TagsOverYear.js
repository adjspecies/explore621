function TagsOverYear(data) {
  ...
  let xScale = d3.scaleTime()
    .domain([Date.parse(data[0].value[0].key + '-01-01'), Date.parse(data[0].value[data[0].value.length - 1].key + '-01-01')])
    .range([100, width]);
  ...
  data.forEach(datum => {
    let line = d3.line()
      .x((d) => xScale(Date.parse(d.key + '-01-01')))
      .y((d) => yScale(d.value));
    vis.append('path')
      .datum(datum.value)
      .style('fill', 'none')
